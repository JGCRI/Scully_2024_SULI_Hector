# Script to create all plots of historical data for SULI presentation
# By: Peter Scully
# Date: 7/15/24
...
T_OUTPUT <- file.path(RESULTS_DIR, "SULI_pres_T_comparison.jpeg")
CO2_OUTPUT <- file.path(RESULTS_DIR, "SULI_pres_CO2_comparison.jpeg")
OHC_OUTPUT <- file.path(RESULTS_DIR, "SULI_pres_OHC_comparison.jpeg")
...
exp11A <- run_hector(ini_file = INI_FILE,
                     ...
                     yrs = 1750:2014,
                     vars = VARS)
exp11A$scenario <- "Hector - Best Parameterization without OHC"
...
exp16 <- run_hector(ini_file = INI_FILE,
                    ...
                    yrs = 1750:2014,
                    vars = VARS)
exp16$scenario <- "Hector - Best New Parameterization"
...
exp11A_OHC <- filter(exp11A, variable == "OHC")
key_data <- rbind(default_data, exp11A_OHC, exp16)       # Best run/comparisons
...
ggplot(data = temp_data, aes(x = year, y = value, color = exp)) + 
  ...
  scale_color_manual(name = "Experiments",
                     values = c("blue", "orange", "snow4", "black"))  + 
  scale_linetype(guide = "none") + 
  theme(legend.text = element_text(size = 15), 
        legend.key.height = unit(2, "cm")) +
  ylab("Temperature Anomaly (\u00B0C)") +
  xlab("Year")
ggsave(T_OUTPUT, width = 12)
...
ggplot(data = ohc_data, aes(x = year, y = value, color = exp)) + 
  ...
  # Cleaning up plot
  scale_color_manual(name = "Experiments",
                     values = c("blue", "#009E73", "orange", "snow4", "black")) + 
  theme(legend.text = element_text(size = 15), 
        legend.key.height = unit(2, "cm")) +
  ylab(expression('Global Ocean Heat Content Anomaly (ZJ)')) +
  xlab("Year")
ggsave(OHC_OUTPUT, width = 12)